...
namespace sakinawedsjuzar.Helper
{
    public class AuthHelper
    {
        ...
        private static string SSOCookieName = Constants.CON_SSOCookieName;
        private static string SSOCookieKeySalt = Constants.CON_SSOCookieKeySalt;
        private static string SSOCookieDomain = Constants.CON_SSOCookieDomain;
        ...
        public static UserModel LoginFromCookie()
        {
            if (AuthenticateSSOCookie())
            {
                ...
                if (ssoCookie != null)
                {
                    string cookie = HttpContext.Current.Request.Cookies["SJSSO"].Value.ToString();
        ...
        public static UserModel StoreUserToSessionEntity(int userId, string emailid)
        {
            ...
            {
                akhbaarUser = Services.GetInstance.GetUserModelByEmailID(emailid);
            }
            /*else if (userId > 0)
            {
        ...
        {
            UserModel user = Services.GetInstance.GetUserModelByEmailID(email);
